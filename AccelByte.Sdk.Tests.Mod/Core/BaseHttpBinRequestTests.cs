...
using AccelByte.Sdk.Tests.Mod.Scenario.Httpbin;

namespace AccelByte.Sdk.Tests.Mod.Core
{
    public abstract class BaseHttpBinRequestTests
    {
        ...
        public void RequestAndResponseBodyTest(int statusCode)
        {
            ...
            HttpbinTestRequest request = new HttpbinTestRequest()
            {
                Name = "test item",
                Weight = 1.2345,
                Quantity = 999,
            ...
            //Monkey patching since current Httpbin server does not provide endpoint with specified status code and response object altogether.
            int monkeyPatchStatusCode = statusCode;

            var result = op.ParseResponse<Dictionary<string, string>, HttpbinTestRequest>(
        ...
        [TestCase("GET", "csv")]
        [TestCase("GET", "multi")]
        public void ArrayQueryStringTest(string method, string collectionFormat)
        {
            Assert.IsNotNull(_Sdk);
            if (_Sdk == null)
                return;
            ...
            // Special characters need to be escaped
            var value = new List<string> { "?a=a&\"?a=a&", "?b=b&\"?b=b&" };
        ...
        [Test]
        [TestCase("POST")]
        public void FileUploadTest(string method)
        {
            Assert.IsNotNull(_Sdk);
            if (_Sdk == null)
                return;
