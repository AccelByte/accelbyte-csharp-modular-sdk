// Copyright (c) 2022-2023 AccelByte Inc. All Rights Reserved.
// This is licensed software from AccelByte Inc, for limitations
// and restrictions contact your company contract manager.

// This code is generated by tool. DO NOT EDIT.


using System;
using System.IO;
using System.Collections.Generic;

using AccelByte.Sdk.Core;
using AccelByte.Sdk.Sample.Cli.Command;

using AccelByte.Sdk.Api.Dsartifact.Wrapper;
using AccelByte.Sdk.Api.Dsartifact.Model;
using AccelByte.Sdk.Api.Dsartifact.Operation;

namespace AccelByte.Sdk.Sample.Cli.ApiCommand.Dsartifact
{
    [SdkConsoleCommand("dsartifact", "listnodesipaddress")]
    public class ListNodesIPAddressCommand : ISdkConsoleCommand
    {
        private IAccelByteSdk _SDK;

        public string ServiceName { get { return "Dsartifact"; } }

        public string OperationName { get { return "ListNodesIPAddress"; } }

        [SdkCommandArgument("limit")]
        public long? Limit { get; set; }

        [SdkCommandArgument("next")]
        public string? Next { get; set; }

        [SdkCommandArgument("nodeIP")]
        public string? NodeIP { get; set; }

        [SdkCommandArgument("previous")]
        public string? Previous { get; set; }

        public ListNodesIPAddressCommand(IAccelByteSdk sdk)
        {
            _SDK = sdk;
        }

        public string Run()
        {
            AccelByte.Sdk.Api.Dsartifact.Wrapper.ArtifactUploadProcessQueue wrapper = new AccelByte.Sdk.Api.Dsartifact.Wrapper.ArtifactUploadProcessQueue(_SDK);

            var opBuilder = AccelByte.Sdk.Api.Dsartifact.Operation.ListNodesIPAddress.Builder;

            if (Limit != null)
                opBuilder.SetLimit((long)Limit);
            if (Next != null)
                opBuilder.SetNext((string)Next);
            if (NodeIP != null)
                opBuilder.SetNodeIP((string)NodeIP);
            if (Previous != null)
                opBuilder.SetPrevious((string)Previous);




            ListNodesIPAddress operation = opBuilder.Build(
            );


            AccelByte.Sdk.Api.Dsartifact.Model.ModelsListNodesIPAddress? response = wrapper.ListNodesIPAddress(operation);
            if (response == null)
                return "No response from server.";

            return SdkHelper.SerializeToJson(response);
        }
    }
}