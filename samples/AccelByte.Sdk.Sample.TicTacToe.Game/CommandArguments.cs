...
namespace AccelByte.Sdk.Sample.TicTacToe.Game
{
    public class CommandArguments
    {
        ...
        public CommandArguments(string[] args)
        {
            ...
            if (args.Length > 0)
            {
                ...
                while (true)
                {
                    ...
                    if (part.Substring(0, Math.Min(2, part.Length)) == "--")
                    {
                        ...
                        {
                            if (aValue == String.Empty)
                                throw new Exception("Server URL is specified but has empty value.");
                            ServerURL = aValue;
                        }
            ...
            if (ServerURL == String.Empty)
            {
                if (Environment.GetEnvironmentVariable("AB_MATCHMAKING_SERVER_URL") != null)
                    ServerURL = Environment.GetEnvironmentVariable("AB_MATCHMAKING_SERVER_URL")!;
            }
        ...
        {
            Console.WriteLine("usage: AccelByte.Sdk.Sample.TicTacToe.Game.exe [options]");
            string opts =
                "\t--help\tShow this help\n"
                + "\t--user\t(optional) Specify username for login operation\n"
                + "\t--pass\t(optional) Specify password for login operation\n"
                + "\t--server\tSpecify TicTacToe sample server URL\n"
